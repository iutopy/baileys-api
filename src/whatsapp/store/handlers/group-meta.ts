	...
	const upsert: BaileysEventHandler<"groups.upsert"> = async (groups) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during groups upsert: ${(e as Error).message}`,
			);
		}
	};
		...
		for (const update of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during group metadata update: ${(e as Error).message}`,
				);
			}
		}
	};
	...
	const updateParticipant: BaileysEventHandler<"group-participants.update"> = async ({
		...
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during group participants update: ${(e as Error).message}`,
			);
		}
	};
